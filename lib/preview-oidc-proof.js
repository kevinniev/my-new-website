...
const PROOF_REVISION = "20260910-oidc-handler-proof-r6";
...
function getManagedPreviewReviewBaseUrl(value) {
  try {
    const url = new URL(value);
    if (url.protocol !== "https:" || !url.hostname.endsWith(".manus.computer")) return null;
    return url.origin;
  } catch {
    return null;
  }
...
export function createProofIdempotencyKey({ deploymentId = "unknown", now = new Date() }) {
  const window = now.toISOString().slice(0, 13);
  return createHash("sha256").update(`${PROOF_SCOPE}:${PROOF_REVISION}:${deploymentId}:${OIDC_PROOF_ROUTE}:${window}`).digest("hex");
}
...
export async function runPreviewOidcProof({
  ...
  const primaryReviewBaseUrl = getManagedPreviewReviewBaseUrl(env.AVF_PRIMARY_REVIEW_BASE_URL);
  if (!env.AVF_AUTOMATION_CLIENT_ID || !env.AVF_AUTOMATION_SIGNING_KEY || !env.VERCEL_URL || !primaryReviewBaseUrl) {
    return jsonError(res, 503, "proof_gateway_not_configured");
  }

  const deploymentId = env.VERCEL_GIT_COMMIT_SHA || env.VERCEL_DEPLOYMENT_ID || env.VERCEL_URL;
  ...
  try {
    const reviewResponse = await fetchImpl(`${primaryReviewBaseUrl}${PRIMARY_REVIEW_PATH}`, {
      method: "GET",
      redirect: "manual",
      headers: {
    ...
    if (reviewResponse.status === 401) return jsonError(res, 502, "primary_review_auth_rejected");
    if (!reviewResponse.ok || !review?.ok) {
      disableResponseCaching(res);
      return res.status(502).json({
        ok: false,
        mode: "fixture_only",
        error: "primary_review_rejected",
        primaryReviewStatus: reviewResponse.status,
      });
    }
