...
function isPreview(env) {
  return env.VERCEL_ENV === "preview" && env.AUTOMATION_MODE === "dry_run";
}
...
export function createProofIdempotencyKey(now = new Date()) {
  const window = now.toISOString().slice(0, 13);
  return createHash("sha256").update(`${PROOF_SCOPE}:${OIDC_PROOF_ROUTE}:${window}`).digest("hex");
}
...
export async function runPreviewOidcProof({
  ...
  const idempotencyKey = createProofIdempotencyKey(now);
