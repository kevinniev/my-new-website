...
/**
 * Search Facebook for posts containing a hashtag.
 * @param {string} hashtag
 * @param {{ limit?: number }} options
 */
export async function searchHashtags(hashtag, { limit = 20 } = {}) {
  const token = process.env.FACEBOOK_PAGE_ACCESS_TOKEN;
  if (!token) return [];
  try {
    const tag = hashtag.replace(/^#/, "");
    const res = await fetch(
      `https://graph.facebook.com/v19.0/search?q=${encodeURIComponent(tag)}&type=post&limit=${limit}&access_token=${token}`,
    ...
    return (data.data || []).map((p) => ({
      ...
      id: p.id,
      text: p.message || "",
      author: { id: p.from?.id, name: p.from?.name },
...
/**
 * Get recent mentions of AVfreelance on X/Twitter.
 * @param {{ limit?: number }} options
 */
export async function getRecentMentions({ limit = 50 } = {}) {
  const bearerToken = process.env.TWITTER_BEARER_TOKEN;
  if (!bearerToken) return [];
  try {
    const res = await fetch(
      `https://api.twitter.com/2/tweets/search/recent?query=avfreelance&max_results=${Math.min(limit, 100)}&tweet.fields=created_at,author_id,text`,
      {
